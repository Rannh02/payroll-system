...
@section('title', 'Create Payroll Run - VIA Architects Associates')
...
<div class="max-w-full mx-auto px-6">
    ...
    <div class="content-header" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:1rem;">
        <div>
            <h2 class="header-title">Create Payroll Run</h2>
        ...
        {{-- Period filter form --}}
        <form method="GET" action="{{ route('finance_admin.payroll.create') }}"
            ...
            <tbody>
                ...
                @php
                    ...
                    $period= $payroll 
                           ...
                           : date('M d', strtotime($from)) . ' – ' . date('M d, Y', strtotime($to));
                @endphp
                <tr class="payroll-row" data-name="{{ strtolower($name) }}" data-dept="{{ $dept }}">
                    ...
                    @else
                        <td colspan="9" style="text-align: center;">
                            <form method="POST" action="{{ route('payroll.run', ['employee' => $emp->employee_id, 'from' => $from, 'to' => $to]) }}" style="margin: 0; display: inline-block;">
                                @csrf
